# Live Trading Bot — MetaTrader 5 / IC Markets (fresh build)

A clean, rule-based trading bot built on the proven MT5 + IC Markets + Telegram
plumbing from the previous project. **The strategy is a blank slate** — the
connection, position sizing, risk controls, CSV logging, Telegram alerts and
backtester are all in place; the entry/exit rules in `strategy.py` are an empty
placeholder awaiting your strategy. Until they're added, the bot connects and
runs but takes **no trades**.

> Setup is identical to the previous bot — see WINE_SETUP.md (run free on a
> Chromebook), ALERTS.md (Telegram), and GO_LIVE.md.
